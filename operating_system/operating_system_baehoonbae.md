...
- **구성**
  - 파일은 기본적으로 파일 이름, 파일을 실행하기 위한 정보, 그리고 파일과 관련된 부가 정보로 구성된다.
- **속성 (Attribute) / 메타데이터 (Metadata)**
  - 파일과 관련된 부가 정보를 파일 속성 또는 메타데이터라고 부른다. 
  - 여기에는 파일의 형식, 위치, 크기, 생성 날짜, 접근 권한 등 파일과 관련된 다양한 정보가 포함된다.
- **운영체제 관리**
  - 파일을 다루는 모든 작업(생성, 삭제, 읽기, 쓰기 등)은 운영체제에 의해 이루어진다.
  - 시스템 콜 사용: 따라서 응용 프로그램은 임의로 파일을 조작하거나 저장할 수 없으며, 반드시 운영체제가 제공하는 파일 관련 시스템 콜(System Call) 을 이용해야만 파일에 접근하고 작업을 수행할 수 있다.

### 파일 디스크립터 (File Descriptor)
- **필요성** 하나의 프로세스가 여러 개의 파일을 동시에 사용하는 경우, 어떤 파일을 대상으로 작업할지 구분할 방법이 필요하다.
- **정의** 운영체제는 현재 특정 프로세스가 사용 중인 파일들을 식별하기 위해 파일 디스크립터(File Descriptor) 라는 특별한 정보를 사용한다. (윈도우 운영체제에서는 파일 핸들(File Handle) 이라고도 부른다.)
- **형태** 파일 디스크립터는 일반적으로 음이 아닌 정수(Non-negative Integer) 이다. (예: 0, 1, 2, 3...) 운영체제는 프로세스가 파일을 열거나 생성할 때마다 고유한 파일 디스크립터 번호를 해당 파일에 할당하고 프로세스에게 알려준다. (예: 표준 입력 0, 표준 출력 1, 표준 에러 2는 미리 할당된 대표적인 파일 디스크립터이다.)
- **사용** 프로그래밍 언어의 종류와 관계없이 파일을 다룰 때 자주 접하게 된다. 특히 유닉스 계열 시스템에서는 파일 식별 및 입출력에 파일 디스크립터를 광범위하게 사용한다.
- **관련 시스템 콜**
  - `open()`: 파일을 열고 해당 파일에 대한 파일 디스크립터를 반환한다.
  - `write()`, `read()`: 특정 파일 디스크립터를 인자로 받아 해당 파일에 데이터를 쓰거나 읽는다.
  - `close()`: 사용이 끝난 파일의 파일 디스크립터를 인자로 받아 파일을 닫는다.

### 디렉터리
- 목적
  - 운영체제는 여러 파일들을 일목요연하게 관리하기 위해 디렉터리(Directory) 를 사용한다.
  - 윈도우 운영체제에서는 이를 폴더(Folder) 라고 부른다.
- **트리 구조**
  - 오늘날의 디렉터리는 대부분 계층적 구조(Hierarchical Structure) 를 가지며, 이는 자료구조 중 트리(Tree) 형태와 같다. 
  - 따라서 트리 구조 디렉터리(Tree-structured Directory) 로 관리된다고 할 수 있다.
  - **최상위 디렉터리 (루트 디렉터리, Root Directory)**: 트리 구조의 가장 꼭대기에 있는 디렉터리이다. 유닉스/리눅스 계열에서는 슬래시(/) 기호로 표현한다. (윈도우는 보통 C:\ 와 같이 드라이브 문자로 시작한다.)
  - **서브 디렉터리 (하위 디렉터리, Subdirectory)**: 루트 디렉터리 또는 다른 디렉터리 안에 포함된 디렉터리이다. 서브 디렉터리 안에 또 다른 서브 디렉터리를 포함할 수 있어 계층 구조를 형성한다.
  - **경로 (Path)**: 트리 구조 내에서 특정 파일이나 디렉터리의 위치를 나타내는 정보이다. 최상위 디렉터리부터 시작하여 중간 디렉터리들을 거쳐 목표 지점까지 이르는 이름을 슬래시(/ 또는 윈도우의 \ )로 구분하여 표현한다. (예: /home/minchul/a.sh)
- **디렉터리 관리 방식**
  - 운영체제는 디렉터리를 일종의 특별한 파일로 간주하여 관리한다.
  - 디렉터리 파일 안에는 그 디렉터리에 속한 파일이나 서브 디렉터리들에 대한 정보가 테이블 형태로 저장된다.
- **디렉터리 엔트리 (Directory Entry)**
  - 디렉터리 내부에 저장되는, 각 파일이나 서브 디렉터리에 대한 정보 한 줄(행)을 디렉터리 엔트리라고 부른다.
  - 각 디렉터리 엔트리에는 최소한 '파일/디렉터리 이름' 과 그 '파일/디렉터리가 보조기억장치 어디에 저장되어 있는지 위치를 유추할 수 있는 정보' 가 포함된다.
  - 이 위치 정보를 통해 운영체제는 디렉터리에 속한 파일을 읽거나, 하위 디렉터리로 이동할 수 있다.
  - **참고**: 흔히 디렉터리 엔트리에서 .은 현재 디렉터리의 위치 정보, ..은 상위 디렉터리의 위치 정보를 가리킨다.
- **디렉터리 엔트리의 추가 정보 (속성)**
  - 일부 파일 시스템에서는 디렉터리 엔트리에 파일 이름과 위치 정보뿐만 아니라 파일 속성(생성 시간, 수정 시간, 크기 등) 정보도 함께 명시하기도 한다.

### 파일 할당
- **블록 (Block)**
  - 운영체제는 파일과 디렉터리를 읽고 쓰는 기본 단위를 블록(Block) 으로 사용한다.
  - 즉, 하나의 파일은 보조기억장치에 저장될 때 하나 이상의 블록을 할당받아 저장된다.
  - 블록 하나의 크기는 보통 4096바이트(4KB) 정도이다.
  - 보조기억장치는 이러한 여러 블록들로 구성되며, 각 블록에는 고유한 주소(블록 번호)가 부여된다.
  - 어떤 파일 시스템을 사용하느냐에 따라 파일 및 디렉터리를 어떤 블록에 어떻게 할당하는지가 달라질 수 있다.

### 파일 할당 방법

#### 연결 할당
- **방식** 
  - 파일을 이루는 각 데이터 블록이 다음 데이터 블록의 주소(포인터)를 가지는 형태로 할당한다. 
  - 마치 연결 리스트(Linked List)와 유사하다. 
  - 파일의 블록들은 디스크 상에 흩어져 있을 수 있다.
- **디렉터리 엔트리 정보** 
  - 연결 할당 방식에서는 디렉터리 엔트리에 일반적으로 파일 이름, 파일 데이터의 첫 번째 블록 주소, 그리고 파일을 이루는 블록 단위의 길이(크기) 가 명시된다.
- **데이터 접근** 
  - 특정 파일의 내용을 읽으려면, 디렉터리 엔트리에서 첫 번째 블록 주소를 확인하고, 해당 블록을 읽은 뒤 그 안에 저장된 다음 블록 주소를 따라가는 방식으로 순차적으로 접근해야 한다. 
  - 예: 파일 a는 10번 블록에서 시작하고 길이가 4블록 -> 10번 읽고 다음 주소 확인 -> 다음 블록 읽고 또 다음 주소 확인...
- **장점** 
  - 외부 단편화가 발생하지 않는다. 
  - 파일 크기가 변해도 유연하게 대처할 수 있다.
- **단점** 
  - 임의 접근(Random Access)이 불가능하고 반드시 순차 접근만 가능하다. 
  - 특정 블록을 찾으려면 처음부터 따라가야 한다. 
  - 블록 내의 포인터 정보 때문에 실제 데이터 저장 공간이 약간 손실된다. 
  - 포인터가 유실되면 파일 전체를 잃을 수 있다.

#### 색인 할당
- **방식**
  - 파일 하나당 색인 블록(Index Block) 이라는 특별한 블록을 둔다. 
  - 이 색인 블록 안에는 해당 파일을 구성하는 모든 데이터 블록들의 주소가 목록 형태로 저장된다. 
  - 데이터 블록 자체는 디스크 상에 흩어져 있을 수 있다.
- **디렉터리 엔트리 정보**
  - 색인 할당 방식에서는 디렉터리 엔트리에 파일 이름과 해당 파일의 색인 블록 주소만 명시하면 된다.
- **데이터 접근**
  - 특정 파일의 데이터를 읽으려면, 디렉터리 엔트리에서 색인 블록 주소를 찾아 해당 색인 블록을 읽는다. 
  - 색인 블록에는 파일의 모든 데이터 블록 주소가 순서대로 들어있으므로, 원하는 위치의 데이터 블록 주소를 바로 얻어 직접 접근(Direct Access)할 수 있다.
- **장점**
  - 외부 단편화가 발생하지 않는다. 
  - 순차 접근과 임의 접근(직접 접근)이 모두 효율적으로 가능하다.
- **단점**
  - 작은 파일이라도 최소한 하나의 색인 블록이 추가로 필요하므로 공간 낭비가 있을 수 있다. 
  - 파일이 매우 클 경우 하나의 색인 블록에 모든 주소를 담지 못할 수 있어, 추가적인 기법(예: 다중 색인 블록)이 필요하다.

## 파일 시스템
- **파일 시스템의 다양성**
  - 파일 시스템의 종류는 매우 다양하며, 운영체제마다 지원하는 파일 시스템이 다르다. 
  - 또한, 같은 운영체제에서도 다른 파일 시스템을 사용하거나 하나의 컴퓨터(보조기억장치)에 여러 파일 시스템을 동시에 사용할 수도 있다.
- **파티셔닝 (Partitioning)과 파티션 (Partition)**
  - 하나의 보조기억장치 내에 여러 종류의 파일 시스템을 사용하려면, 보조기억장치에 파일 시스템을 적용할 영역을 미리 구분해야 한다.
  - 보조기억장치의 영역을 이렇게 나누는 작업을 파티셔닝(Partitioning) 이라고 하며, 파티셔닝되어 나누어진 하나의 영역을 파티션(Partition) 이라고 부른다.
  - 즉, 하나의 보조기억장치는 여러 파티션으로 나눌 수 있으며, 각 파티션마다 다른 파일 시스템을 설치하여 사용할 수 있다. (윈도우 디스크 관리자 스크린샷이 그 예시이다.)
- **포맷팅 (Formatting)**
  - 어떤 파일 시스템을 사용할지는 주로 보조기억장치를 포맷(Formatting) 할 때 결정한다.
  - 포맷팅이란 특정 파티션(또는 디스크 전체)에 어떤 파일 시스템을 설정할지 결정하고 해당 파일 시스템의 방식으로 파일을 저장할 수 있도록 디스크 구조를 준비하는 작업이다. (USB 메모리 포맷이나 리눅스 설치 시 파티션 설정 및 포맷 화면이 그 예시이다.)
- **운영체제별 주요 파일 시스템**
  - 파일 시스템마다 제공되는 기능, 특성, 성능, 지원하는 최대 파일 크기 등이 다를 수 있다.
  - **윈도우**: NT 파일 시스템(NTFS), Resilient File System(ReFS) 등
  - **리눅스**: EXT, EXT2, EXT3, EXT4, XFS, ZFS 등
  - **macOS**: APFS(Apple File System) 등
- **아이노드 (Inode)**
  - 개발자가 자주 접하게 되는 많은 파일 시스템(특히 리눅스 계열의 EXT, XFS, ZFS 등)에서는 아이노드(inode) 라는 개념을 기반으로 파일을 할당하고 관리한다.
  - 아이노드는 파일의 메타데이터(Metadata) 를 담고 있는 자료 구조이다. 
  - 여기에는 파일이 실제 저장된 위치 정보와 다양한 속성 정보(파일 소유자, 권한, 크기, 시간 정보 등 파일 이름을 제외한 거의 모든 것) 가 담겨 있다.

### 아이노드 기반 파일 시스템
- **아이노드 (Inode, Index-Node)**
  - 아이노드 기반 파일 시스템에서는 파일이나 디렉터리마다 각각 고유한 아이노드를 가진다.
  - 이 아이노드에는 고유한 번호(아이노드 번호)가 부여된다.
  - 아이노드는 파일의 메타데이터(속성 정보 및 데이터 저장 위치 정보)를 담고 있는 중요한 자료 구조이다. (단, 파일 이름은 아이노드 자체가 아닌 디렉터리 엔트리에 저장된다.)
- **아이노드 번호 확인**
  - 리눅스나 macOS 같은 운영체제에서는 `ls -li` 명령어 등을 통해 파일 목록을 조회할 때 파일 속성 정보 앞부분에 나오는 숫자가 바로 해당 파일/디렉터리의 아이노드 번호이다.

#### EXT4 파일 시스템 구조
- **개요**
  - EXT4 파일 시스템으로 포맷된 파티션은 대략 다음과 같은 구조로 구성된다.
- **주요 구성 요소**
  - **부트 블록 (Boot Block)**: 파티션의 가장 앞부분에 위치하며, 부팅 관련 정보나 파티션 관리 정보를 담는다. (실제 파일 데이터 저장 영역 아님)
  - **블록 그룹 (Block Group)**: 부트 블록 이후 영역을 여러 개의 블록 그룹으로 나누어 관리하며, 각 그룹은 독립적인 파일 시스템 정보를 가진다.
- **각 블록 그룹의 구성 요소**
  - **슈퍼블록 (Superblock)**: 전체 파일 시스템 정보(총 아이노드/블록 개수, 블록 크기 등)를 담는 설정값. (안정성을 위해 여러 그룹에 복사본 저장)
  - **그룹 서술자 (Group Descriptors)**: 해당 블록 그룹의 메타데이터(비트맵/테이블 위치, 남은 블록/아이노드 수 등) 저장.
  - **블록 비트맵 (Block Bitmap)**: 그룹 내 데이터 블록 사용 여부(사용 중/비어 있음)를 비트로 표시.
  - **아이노드 비트맵 (Inode Bitmap)**: 그룹 내 아이노드 사용 여부(사용 중/비어 있음)를 비트로 표시.
  - **아이노드 테이블 (Inode Table)**: 그룹 내 아이노드들의 실제 정보(메타데이터)가 순차적으로 저장되는 영역.
  - **데이터 블록 (Data Blocks)**: 파일의 실제 내용(데이터)이 저장되는 블록.

#### 아이노드 할당의 특징
- **미리 할당**
  - 아이노드는 파티션 내 특정 영역(아이노드 테이블)에 미리 할당되는 경우가 많다.
- **아이노드 고갈 문제**
  - 데이터 블록 공간이 남아 있어도, 할당 가능한 아이노드가 없으면(아이노드 테이블이 꽉 차면) 새 파일 생성이 불가능할 수 있다.

### 마운트
- **개념**: 마운트란 어떤 저장장치(예: USB 메모리)의 파일 시스템을 다른 저장장치(예: 컴퓨터의 주 하드디스크)의 파일 시스템의 특정 디렉터리에 연결하여, 마치 원래 그 디렉터리에 속해 있었던 것처럼 접근할 수 있도록 파일 시스템을 편입시키는 작업을 의미한다.
- **예시**: USB 메모리를 데스크톱 컴퓨터에 연결하면, 데스크톱 컴퓨터의 파일 탐색기를 통해 USB 메모리 안의 파일과 폴더에 접근할 수 있다. 이것이 가능한 이유는 USB 메모리의 파일 시스템이 데스크톱 컴퓨터 파일 시스템의 특정 지점(예: 윈도우의 특정 드라이브 문자 또는 리눅스의 특정 디렉터리)에 마운트되었기 때문이다.
- **동작 방식**:
    - 하나의 파일 시스템(A)이 다른 파일 시스템(B)의 특정 디렉터리(**마운트 지점, Mount Point**)에 마운트되면, 파일 시스템 A의 루트 디렉터리가 파일 시스템 B의 마운트 지점 디렉터리처럼 취급된다.
    - **예시**: USB 메모리의 파일 시스템(루트 `/` 아래에 `homework`, `pictures`, `video` 디렉터리가 있음)을 데스크톱 컴퓨터의 `/mnt` 디렉터리에 마운트하면,
        - 데스크톱 컴퓨터의 `/mnt` 경로로 접근하면 이제 USB 메모리의 루트 디렉터리로 접근하는 것과 같아진다.
        - 따라서 USB 메모리 안의 `homework/os/a.cpp` 파일은 데스크톱 컴퓨터에서는 `/mnt/homework/os/a.cpp` 경로로 접근할 수 있게 된다.
        - 마치 USB 메모리의 파일 시스템 전체가 데스크톱 컴퓨터의 `/mnt` 디렉터리 아래에 연결된 것처럼 보인다.
- **마운트/마운트 해제**:
    - 운영체제의 그래픽 사용자 인터페이스(GUI) 설정 메뉴(예: macOS의 디스크 유틸리티)를 통해 간단히 마운트하거나 마운트 해제(추출)할 수 있다.
    - 명령어 기반(CLI)으로도 마운트 작업을 수행할 수 있다. 유닉스/리눅스 시스템에서는 `mount` 명령어를 자주 사용한다.
        - **명령어 예시**: `mount -t ext4 -o ro /dev/sda /mnt/test`
        - **의미**: `/dev/sda` 라는 장치(파티션)에 있는 ext4 파일 시스템을 `/mnt/test` 라는 디렉터리에 읽기 전용(ro) 옵션으로 마운트하라는 명령어이다.

마운트는 다른 저장 장치나 파티션의 파일 시스템을 현재 사용 중인 파일 시스템 계층 구조의 일부로 통합하여 편리하게 접근하고 사용할 수 있게 해주는 중요한 기능이다.
...
- **설계 목표**: CPU 명령어 파이프라이닝은 명령어 처리 속도 향상을 위해 설계된 기술이다.
    - 명령어 처리 과정을 여러 단계로 나누어 각 단계가 하드웨어적으로 동시에 다른 명령어를 처리하도록 한다.
    - 이를 통해 **명령어 수준 병렬성(Instruction-Level Parallelism, ILP)** 을 달성하는 것을 목표로 한다.
- **핵심 원리**: **독립적인 하드웨어 단계**
    - 각 단계(IF, ID, EX 등)는 별도의 하드웨어 로직으로 구현되어 있다.
    - 이 때문에 한 클럭 사이클에 IF 유닛은 다음 명령어를 가져오고, ID 유닛은 그 이전 명령어를 해독하며, EX 유닛은 그 전전 명령어를 실행하는 등, 여러 단계의 작업이 **동시에** 이루어질 수 있다.
- **비유**: **자동차 조립 라인**
    - 각 공정(단계)이 동시에 다른 차체에 대해 작업을 수행한다 (예: 차체 올리기, 문 달기, 엔진 넣기).
    - 전체 라인이 가동되면, 매 단위 시간마다 완성된 차가 하나씩 나온다 (비록 차 한 대를 완성하는 데는 긴 시간이 걸리더라도).
    - 이 조립 라인은 시간을 나누어 쓰는 방식(동시성)이 아니라, 각 공정이 **병렬적으로 동시에 작동**하도록 설계된 것이다.
- **결론**
    - 파이프라인 해저드(Hazard)와 같은 요인으로 인해 파이프라인이 잠시 멈추거나(stall) 지연될 수는 있다.
    - 하지만 하드웨어 구조 자체는 근본적으로 여러 단계의 작업을 **병렬적으로 동시에 수행**하도록 설계되었다.
    - 이것이 파이프라이닝이 동시성(Concurrency)보다는 **병렬성(Parallelism)**에 더 가까운 핵심적인 이유이다.